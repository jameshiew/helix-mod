...
//!
//! Language servers do not log through here: each server's stderr is written
//! verbatim to its own file, see `helix_loader::lsp_log_file`.
...
/// Minimal `log::Log` implementation: a level filter plus a single sink.
struct Logger {
